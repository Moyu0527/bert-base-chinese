...
# 输入层 Embeddings
draw_box(10, 15, 12, 6, "字词嵌入\n(Token Emb)", "#E8F0FE", "#4A76A8", fontsize=11)
draw_box(25, 15, 12, 6, "句段嵌入\n(Segment Emb)", "#E8F0FE", "#4A76A8", fontsize=11)
draw_box(40, 15, 12, 6, "位置嵌入\n(Position Emb)", "#E8F0FE", "#4A76A8", fontsize=11)
...
# BERT 堆叠层
_, top_in, _, _ = draw_box(25, 33, 22, 6, "输入表征\n(Input Representation)", "#F5F5F5", "#888888")
draw_arrow((25, 27), (25, 30))

_, top_t1, _, _ = draw_box(25, 45, 26, 8, "Transformer 编码器 1", "#E6F4EA", "#57A66C", fontsize=14)
draw_arrow((25, 36), (25, 41))

ax.text(25, 55, "......", fontsize=24, ha='center', va='center', color="#888888", rotation=90)
draw_arrow((25, 49), (25, 52))

_, top_tn, _, _ = draw_box(25, 65, 26, 8, "Transformer 编码器 12", "#E6F4EA", "#57A66C", fontsize=14)
draw_arrow((25, 58), (25, 61))

draw_box(25, 78, 22, 6, "语境化输出向量\n(Contextual Embeddings)", "#FCE8E6", "#C25953", fontsize=12)
...
# 1. Input
bot_enc_in, top_enc_in, left_enc_in, _ = draw_box(75, 20, 14, 5, "输入\n(Input)", "#FFFFFF", "#888888")
...
bot_mha, top_mha, _, _ = draw_box(75, 48, 30, 8, "多头自注意力机制\n(Multi-Head Attention)", "#E8F0FE", "#4A76A8", fontsize=13)
# Q, K, V -> MHA
...
# 4. Add & Norm 1
bot_an1, top_an1, left_an1, _ = draw_box(75, 60, 18, 5, "残差相加与归一化\n(Add & Norm)", "#FCE8E6", "#C25953", fontsize=10)
...
ax.text(60, 40, "残差连接\n(Residual)", fontsize=10, color="#C25953", rotation=90, va='center')

# 5. Feed Forward
bot_ffn, top_ffn, _, _ = draw_box(75, 72, 22, 6, "前馈神经网络\n(Feed Forward)", "#E8F0FE", "#4A76A8", fontsize=13)
draw_arrow((75, 62.5), (75, 69))

# 6. Add & Norm 2
bot_an2, top_an2, left_an2, _ = draw_box(75, 84, 18, 5, "残差相加与归一化\n(Add & Norm)", "#FCE8E6", "#C25953", fontsize=10)
...
plt.savefig('results/BERT_Architecture_Attention.svg', format='svg', bbox_inches='tight')
